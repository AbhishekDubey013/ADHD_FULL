...
//               typingIndicator={isTyping ? <TypingIndicator content="Nurture is typing" /> : null}
//             >
...
const systemMessage = {
  role: 'system',
  content: "a mock interview with chatgpt as psychologist and user as adhd patient, ask of history, past experiences traumatic experiences ask question one by one and next question ask should be based on previous response provide. you have a total 7 questions to ask and self-survey questionnaire is already performed and provided to you, focus on areas with rating as very often and often, don't say that you are AI",
};

function App() {
  const [messages, setMessages] = useState([
    {
      message: "Hi there, let's start the session when you're ready. Please tell me your age to get started!",
      sentTime: 'just now',
      sender: 'ChatGPT',
    },
  ]);
  ...
  const apiCallToStoreDataInServer = async () => {
    try {
      ...
      const response = await fetch('https://gt-7tqn.onrender.com/api/auth/chat', {
        ...
        body: JSON.stringify({ mobileNumber, dataArray: messages }),
      });
  ...
  return (
    <div className="App">
      <div style={{ position: 'center', height: '700px', width: '100%' }}>
